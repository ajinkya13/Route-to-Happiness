...
def k_shortest_paths(G, source, target, k=4, weight='weight'):
...
G=nx.Graph()
G = nx.read_gml("UpdatedGraph.gml")

kshortest=(k_shortest_paths(G, int(sys.argv[1]), int(sys.argv[2]), 10))
print kshortest
paths=kshortest[1]
lengths=kshortest[0]
i=0
j=0
maxsentiment=0
shortestpathsentiment=0

for n in paths[0]:
    shortestpathsentiment=shortestpathsentiment+G.node[n]['sentiment']
    

for path in paths:
    if lengths[i]<=lengths[0]+(lengths[0]*0.1):
        sentiment=0
        for n in path:
            sentiment=sentiment+G.node[n]['sentiment']
